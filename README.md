...
Because those are real values, they are written into the prompt as data rather than as prose: JSON quoted, so a value cannot escape onto a line of its own, and length capped, with one sentence in the instructions saying that anything in quotes on a column line is a value the source holds and is not an instruction. That is defence in depth and not a guarantee — a model can be argued with inside quotes too — and what it bounds is the value of a `status` column somebody wrote a sentence into. It changes nothing about the syntax: the answer is a schema validated IR, names resolve against the catalog inside the configured scope, and values are bound as typed parameters, so no string out of a table becomes SQL.
