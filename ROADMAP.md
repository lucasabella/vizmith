...
**Profiling is SQL that the catalog executes.** The profiler writes one query and the catalog runs it, so a second source means teaching the catalog to execute rather than writing a second profiler. The parts of SQL that differ between sources are a `Dialect` on the catalog: how an identifier is quoted, how a set of distinct values is collected, and the approximate distinct count where the source has one. They are templates rather than function names, because the shape differs as well as the name. This costs a `run` on the catalog interface that the Databricks implementation does not have yet, since the warehouse round trip belongs to the query builder, so today the fixture catalog in the tests is the only thing that implements it. The alternative was a profiler that knows Unity Catalog, which would have to be written twice the day a second source arrives.
